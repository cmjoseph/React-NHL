	...
	constructor(){
  	...
  	handleClickPlayer(){
        ...
        if (this.state.playing === false) {
        	audio.play();
        	// title.start();
        	this.setState({playing: true, waves: 'active'});
        } else {
        	audio.pause();
        	// title.stop();
        	this.setState({playing: false, waves: ''});
        }
    }
	...
	render(){
	    return(
	    	<div className="audio__container">
		    	<div className="audio__play" onClick={this.handleClickPlayer}>
					<button id="playbtn" className={this.state.waves}></button>
					<marquee id="songname" behavior="scroll">{this.props.artist} - {this.props.song}</marquee>
					<svg id="wave" className={this.state.waves} data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 38.05">
